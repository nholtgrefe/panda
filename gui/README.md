...
## Installation
The GUI is available as an executable for Linux and Windows [here](https://drive.google.com/drive/folders/1QQXV0h1XUMIXVlNTy5vgyK0DtA-QtJTd?usp=drive_link). To install and run, simply download the `.exe` file for your operating system and open/execute it once downloaded. (If preffered, the GUI can also be run from the [source files](https://github.com/nholtgrefe/panda/tree/main/gui/src), although we then suggest directly using our Python package [`phypanda`](https://github.com/nholtgrefe/panda/tree/main/phypanda) instead.)
